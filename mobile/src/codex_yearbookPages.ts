  ...
  if (kind === "index") {
    ...
    entries.forEach((entry) => {
      ...
      while (offset < lines.length) {
        ...
        const capacity = continuation ? 28 : 22;
        pages.push({ kind, title: continuation ? "作品与感受 · 续页" : "作品与感受", lines: lines.slice(offset, offset + capacity), entryIds: [entry.id], entry, continuation });
        offset += capacity;
      }
    });
  }
...
export async function renderJournalPage(year: number, entries: ReviewEntry[], page: JournalImagePage, index: number, total: number, now = new Date(), options: JournalImageOptions = {}) {
  ...
  if (page.kind === "cover") {
    ...
    counts.forEach((count, i) => {
      ...
      text(`${i + 1}月`, x + 39, baseline + 48, 26, "#68726d");
      ctx.textAlign = "left";
    });
    text("0：暂无记录    —：月份未到", 72, 1055, 28, "#68726d");
    ...
    if (firstWork) await drawJournalCover(ctx, page.entry!, 72, 270, 205);
    const start = firstWork ? 530 : 305;
    page.lines.forEach((line, i) => text(line, 72, start + i * LINE_HEIGHT));
  }
