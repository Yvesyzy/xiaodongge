...
try {
  ...
  if ($packageLine -notmatch "versionCode='25'") { throw "Unexpected APK versionCode metadata: $packageLine" }
  if ($packageLine -notmatch "versionName='2\.8\.0'") { throw "Unexpected APK versionName metadata: $packageLine" }
  $releaseDirectory = Join-Path $root "release"
  New-Item -ItemType Directory -Path $releaseDirectory -Force | Out-Null
  $publishedApk = Join-Path $releaseDirectory "codex_xiaodongge-v2.8.0.apk"
